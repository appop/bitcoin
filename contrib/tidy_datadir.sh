#!/bin/bash
# Copyright (c) 2013 The nealcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete nealcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no nealcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old nealcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected nealcoin 0.7 datadir."
    ;;
  3)
    echo "Detected nealcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected nealcoin 0.8 datadir."
    ;;
esac
