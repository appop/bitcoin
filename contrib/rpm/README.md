...
The RPM spec file provided here is for nealcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://nealcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[nealcoin git repository](https://github.com/nealcoin/nealcoin) but are not part of
...
Currently only `Source10` is used, for the example `nealcoin.conf` file.
...
in the nealcoin git repository. At present only an SVG version of the nealcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the nealcoin
...
needed when 0.12.1 is released, a proper fix is already in the nealcoin git
master branch.
...
idea to build nealcoin Core against a static version of that library compiled
...
The `nealcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `nealcoin-server` package. At this point, most RPM based
...
The `nealcoin-server` package should have SELinux support. How to properly do
...
nealcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/nealcoin/). He has been
packaging nealcoin for Fedora at least since 2012.
...
4. I renamed the `nealcoin` package that contains the Qt GUI to `nealcoin-core` as
that appears to be how the general population refers to it, in contrast to
`nealcoin-xt` or `nealcoin-classic`. I wanted to make sure the general population
