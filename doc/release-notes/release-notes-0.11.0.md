nealcoin Core version 0.11.0 is now available from:

  <https://nealcoin.org/bin/nealcoin-core-0.11.0/>
...
For example, add the following to `nealcoin.conf`:
...
related to the blockchain in the nealcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows nealcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that nealcoin Core will 
...
For an example script see [nealcoin-submittx](https://github.com/laanwj/nealcoin-submittx).
...
- #5334 `cf87536` libnealcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new nealcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only nealcoinAmountField
- #5753 `0f44672` Add nealcoin logo to about screen
...
- #5809 `46bfbe7` Add nealcoin-cli man page
...
- #5849 `41259ca` contrib/init/nealcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename nealcoin-Qt.app to nealcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for nealcoin-tx)
